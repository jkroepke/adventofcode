...
(async () => {
  ...
  for (const panelLine of Object.keys(panel.panel).sort((a, b) => parseInt(a) - parseInt(b))) {
    const length = Object.keys(panel.panel[panelLine]).length;
    if (length > maxLength) {
      maxLength = Object.keys(panel.panel[panelLine]).length;
    }
  }

  for (const panelLine of Object.keys(panel.panel).sort((a, b) => parseInt(a) - parseInt(b))) {
    for (let i = 0; i <=maxLength; i++) {
      process.stdout.write(panel.panel[panelLine][i] ? '#' : ' ');
    }
