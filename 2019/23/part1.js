...
class Network {
  ...
  async handlePackets() {
    ...
    outerLoop:
      while(true) {
        const sendEmptyPacket = [...this.computers.keys()];
        ...
        for (let address = 0; address < computerCount; address++) {
          ...
          if(targetAddress === 255) {
            console.log("First packet to 255 ", x, y);
            break outerLoop;
          }
          ...
          this.computers[targetAddress].input.push(x);
          this.computers[targetAddress].input.push(y);

          const index = sendEmptyPacket.indexOf(targetAddress);
          if (index !== -1) sendEmptyPacket.splice(index, 1);
        }

        for (const targetAddress of sendEmptyPacket) {
          if(this.computers[targetAddress].input.length === 0) {
            this.computers[targetAddress].input.push(-1);
          }
        }
