...
class Guy {
  constructor(intCode) {
    ...
    this.screen = {'0':{'0':this.state.empty}};
    this.x = 0;
    this.y = 0;
  ...
  async run() {
    while(true) {
      ...
      if (status === 0) {
        this.screen[y][x] = this.state.wall;
      } else if (status === 1) {
        this.screen[y][x] = this.state.empty;
      } else if (status === 2) {
        this.screen[y][x] = this.state.oxygenGenerator;
      }
      this.tile[`${x},${y}`] = this.screen[y][x];

      if(this.screen[y][x] === this.state.oxygenGenerator) {
        this.oxygen.push(`${x},${y}`);
      } else if(this.screen[y][x] === this.state.empty) {
        this.empty.push(`${x},${y}`);
      }
  ...
  async calculateOxygen() {
    ...
    while(this.oxygen.length) {
      const newOxygen = [];

      for (const position of this.oxygen) {

        const [x, y] = position.split(',').map(Number);

        this.tile[position] = this.state.oxygen;

        this.screen[y][x] = this.state.oxygen;
        ...
        const lookRight = `${x+1},${y}`;
        // Top
        if (this.tile[lookTop] === this.state.empty) {
          newOxygen.push(lookTop);
        }
        // Bottom
        if (this.tile[lookBottom] === this.state.empty) {
          newOxygen.push(lookBottom);
        }
        // Left
        if (this.tile[lookLeft] === this.state.empty) {
          newOxygen.push(lookLeft);
        }
        // Right
        if (this.tile[lookRight] === this.state.empty) {
          newOxygen.push(lookRight);
        }
      }
    ...
    console.log('x minutes until room is full: '+ minute);
  }
}
